 ...
 */
public class FillupDB extends SQLiteOpenHelper {
    private static final boolean debugon = false;
    private static final int DATABASE_VERSION = 3;
    private static String dbName = "fillup";
    private String dbPath;
    private SQLiteDatabase crsDB;
    private final Context context;
    private static FillupDB sInstance;

    public static synchronized FillupDB getInstance(Context context) {
        // Use the application context, which will ensure that you
        // don't accidentally leak an Activity's context.
        // See this article for more information: http://bit.ly/6LRzfx
        if (sInstance == null) {
            sInstance = new FillupDB(context.getApplicationContext());
        }
     ...
     * @param context
     */
    public FillupDB(Context context) {
    ...
    public void copyDB() throws IOException {
        try {
            if (!checkDB()) {
                ...
                debug("FillupDB --> copyDB", "checkDB returned false, starting copy");
                InputStream ip = context.getResources().openRawResource(R.raw.fillup);
            ...
            Log.w("FillupDB --> copyDB", "IOException: " + e.getMessage());
        }
    }
    ...
    public SQLiteDatabase openDB() throws SQLException {
        ...
        debug("FillupDB --> openDB", "opened db at path: " + crsDB.getPath());
        return crsDB;
    }
    ...
    private boolean checkDB() {    //does the database exist and is it initialized?
        ...
        try {
            ...
            debug("FillupDB --> checkDB: path to db is", path);
            ...
            if (aFile.exists()) {
                ...
                if (checkDB != null) {
                    debug("FillupDB --> checkDB", "opened db at: " + checkDB.getPath());
                    Cursor tabChk = checkDB.rawQuery("SELECT name FROM sqlite_master where type='table' and name='fillup';", null);
                    boolean crsTabExists = false;

                    if (tabChk == null) {
                        debug("FillupDB --> checkDB", "check for fillup table result set is null");
                    } else {
                        tabChk.moveToNext();
                        debug("FillupDB --> checkDB", "check for fillup table result set is: " + ((tabChk.isAfterLast() ? "empty" : tabChk.getString(0))));
                        crsTabExists = !tabChk.isAfterLast();
                    }

                    if (crsTabExists) {
                        Cursor c = checkDB.rawQuery("SELECT * FROM fillup", null);
                        c.moveToFirst();

                        while (!c.isAfterLast()) {
                            int id = c.getInt(0);
                            debug("FillupDB --> checkDB", "Fillup table has Fillup: " + id);
                            c.moveToNext();
                        }
            ...
            Log.w("FillupDB->checkDB", e.getMessage());
        }
